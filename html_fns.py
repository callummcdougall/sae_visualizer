...
from utils_fns import to_str_tokens
...
def generate_tok_html(
    ...
    html_output = re.sub("pos_str_(\d)", lambda m: pos_str[int(m.group(1))].replace(" ", "&nbsp;"), html_output)
    html_output = re.sub("neg_str_(\d)", lambda m: neg_str[int(m.group(1))].replace(" ", "&nbsp;"), html_output)
    html_output = re.sub("pos_val_(\d)", lambda m: f"{pos_val[int(m.group(1))]:+.3f}", html_output)
    html_output = re.sub("neg_val_(\d)", lambda m: f"{neg_val[int(m.group(1))]:+.3f}", html_output)

    # If the effect on loss is nothing (because feature isn't active), replace the HTML output with smth saying this
    if is_empty:
        html_output = (
            html_output
            .replace('<div class="half-width-container">', '<div class="half-width-container" style="display: none;">')
            .replace('<!-- No effect! -->', '<p style="font-size:0.8em;">Feature not active on prev token;<br>no predictions were affected.</p>')
            .replace('<div class="tooltip">', '<div class="tooltip" style="height:175px; width:250px;">')
        )
    # Also, delete the columns as appropriate if the number is between 0 and 5
    else:
        html_output = html_output.replace('<tr><td class="right-aligned"><code></code></td><td class="left-aligned">+0.000</td></tr>', "")
